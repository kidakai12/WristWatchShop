...
$db=new connect();
    if(isset($_GET['action']))
    {
        ...
        if($_GET['action']=="fix") 
        {
            ...
            $rowkhachang = $db->chen("SELECT * FROM `products` WHERE id=$id");
            $tensp; $danhcho; $gia;$hinhanh;$danhchonguoc;
            while($row1=$rowkhachang->fetch()) 
            {
                    $tensp=$row1['name'];
                    $danhcho=$row1['danhcho'];
                    $gia=$row1['price'];
                    $hinhanh=$row1['image'];
            }
            if($danhcho=="Nam"){
        ...
        if($_GET['action']=="delete") {
            ...
            $id=(int)$_GET['id'];
             //XÓA KHÁCH HÀNG ĐÓ
            $db2->exec("DELETE FROM `products` WHERE id=$id");
            //THÔNG BÁO XÓA VÀ CHUYỂN VỀ TRANG DANH SÁCH
            echo "<script>alert('Đã xóa sản phẩm thành công');window.location.href='adminproduct.php';</script>";
            exit;
        }
    }
?>
        ...
        <div class="wrapcontainer">
            <div class="container">
                <form action="controller/controllerADMINproduct.php" method="POST" enctype="multipart/form-data">
                    <h1>EDIT Thông Tin Sản Phẩm</h1>
                <div class="addNV">
                    <div class="input">
                        <div class="khung"><a href="">Hình Ảnh</a>
                        <img width="200" height="40" src="<?php echo $hinhanh; ?>" alt="">
                        ...
                        <div style="margin-left: 250px;">
                            <input type="radio" name="gioitinh" checked value="<?php echo $danhcho; ?>"><?php echo $danhcho; ?>
                            <input type="radio" name="gioitinh" value="<?php echo $danhchonguoc; ?>"><?php echo $danhchonguoc; ?>
                        </div>
                        </div>
                        ...
                        <button id="btn" class="btn btn-outline-secondary" name="action" value="sua">SỬA</button>
                    </div>
                </div>
                </form>
            </div>
